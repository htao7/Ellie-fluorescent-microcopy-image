...
def FindLabels(img,labels,r,c):
    ...
    for i in labels_cor:
        if (i[0] > r / 2 and i[1] < c / 2):
            ...
            br = i
    bl_region = np.copy(img[bl[0] - 50 : bl[0] + 50,bl[1] - 50 : bl[1] + 50])
    _,bl_region = cv2.threshold(bl_region,20,255,cv2.THRESH_BINARY)
    br_region = np.copy(img[br[0] - 50 : br[0] + 50,br[1] - 50 : br[1] + 50])
    _,br_region = cv2.threshold(br_region,20,255,cv2.THRESH_BINARY)
    tl_region = np.copy(img[tl[0] - 50 : tl[0] + 50,tl[1] - 50 : tl[1] + 50])
    _,tl_region = cv2.threshold(tl_region,10,255,cv2.THRESH_BINARY)
    
    bl_well = cv2.HoughCircles(bl_region, cv2.HOUGH_GRADIENT, 1, 200, \
                             param1=150, param2=5, minRadius=21, maxRadius=23)
    ...
    radius = 0.9 * (bl_well[0][0][2] + br_well[0][0][2] + tl_well[0][0][2]) / 3
